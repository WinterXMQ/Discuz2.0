 ...
 *
 *      $Id: class_forumupload.php 32057 2012-11-05 08:16:03Z monkey $
 */
...
class forum_upload {
	...
	function uploadmsg($statusid) {
		...
		if($this->simple == 1) {
			...
			echo 'DISCUZUPLOAD|'.($_G['gp_type'] == 'image' ? '1' : '0').'|'.$statusid.'|'.$this->aid.'|'.$this->attach['isimage'].'|'.($this->attach['isimage'] ? $this->attach['attachment'] : '').'|'.$this->attach['name'].'|'.$this->error_sizelimit;
		} else {
