 ...
 *
 *      $Id: admincp_cloudaddons.php 32211 2012-11-30 03:41:37Z monkey $
 */
...
if(!$operation) {
	...
	if($step == 0) {
		...
		if($_GET['type'] == 'plugin') {
			$plugin = DB::fetch_first("SELECT pluginid FROM ".DB::table('common_plugin')." WHERE identifier='$_GET[key]'");
			if(!$plugin['pluginid']) {
