...
function block_updatecache($bid, $forceupdate=false) {
	...
	if(is_object($obj)) {
		...
		if(in_array($block['blockclass'], array('forum_thread', 'group_thread', 'space_blog', 'space_pic', 'portal_article'))) {
			...
			$query = DB::query('SELECT * FROM '.DB::table('common_block_item_data')." WHERE bid='$bid' AND isverified='1' $bannedsql ORDER BY stickgrade DESC, verifiedtime DESC LIMIT ".($shownum*2));
			while(($value=DB::fetch($query))) {
